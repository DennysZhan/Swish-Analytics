...
class SpeedAndDistanceCalculator:
    ...
    def calculate_distance(self, tactical_player_positions):
        ...
        for frame_number, tactical_player_positions_frame in enumerate(tactical_player_positions):
            output_distances.append({})

            for player_id, current_player_position in tactical_player_positions_frame.items():
                if player_id in previous_players_positions:
                    previous_player_position = previous_players_positions[player_id]
    ...
    def calculate_speed(self, distances, fps=30):
        ...
        for frame_index in range(len(distances)):
            ...
            for player_id in distances[frame_index].keys():
                ...
                for i in range(start_frame, frame_index + 1):
                ...
                if frames_present > window_size:
                    ...
                    if time_in_hours > 0:
                        ...
                        speed[frame_index][player_id] = speed_kmh
                    else:
                        speed[frame_index][player_id] = 0
                else:
                    speed[frame_index][player_id] = 0
